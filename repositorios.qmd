# Repositórios {#sec-repositorios}

Nesta seção, listamos e descrevemos todos os repositórios utilizados pelo Painel Gerencial CIGA-MPMG.

## painelMain

- **Descrição**: contém o código-fonte da home do Painel Gerencial CIGA-MPMG (@sec-painel-home), a página acessada logo após o login.

- **Link**: [https://github.com/ciga-mpmg/painelMain](https://github.com/ciga-mpmg/painelMain)

- **Actions**: possui os Actions de deploy em homologação e produção de cada um dos painéis e para gerar os relatórios gerenciais.

## painelBarragens

- **Descrição**: contém o código-fonte do painel Barragens (@sec-painel-barragens).

- **Link**: [https://github.com/ciga-mpmg/painelBarragens](https://github.com/ciga-mpmg/painelBarragens)

- **Actions**: não possui.

## painelInfoTacs

- **Descrição**: contém o código-fonte do painel Informações sobre os TACs (@sec-painel-infoTacs).

- **Link**: [https://github.com/ciga-mpmg/painelInfoTacs](https://github.com/ciga-mpmg/painelInfoTacs)

- **Actions**: não possui.

## painelDescaracterizacao

- **Descrição**: contém o código-fonte do painel Descaracterização (@sec-painel-descaracterizacao).

- **Link**: [https://github.com/ciga-mpmg/painelDescaracterizacao](https://github.com/ciga-mpmg/painelDescaracterizacao)

- **Actions**: não possui.

## painelRecomendacoes

- **Descrição**: contém o código-fonte do painel Recomendações (@sec-painel-recomendacoes).

- **Link**: [https://github.com/ciga-mpmg/painelRecomendacoes](https://github.com/ciga-mpmg/painelRecomendacoes)

- **Actions**: não possui.

## painelAssets

- **Descrição**: contém o código-fonte de um pacote R com funções auxiliares utilizadas em todos os painéis.

- **Link**: [https://github.com/ciga-mpmg/painelAssets](https://github.com/ciga-mpmg/painelAssets)

- **Actions**: não possui.

## painelLogin

- **Descrição**: contém o código-fonte de um pacote R com funções utilizadas na construção da camada de login via Firebase de cada Painel.

- **Link**: [https://github.com/ciga-mpmg/painelLogin](https://github.com/ciga-mpmg/painelLogin)

- **Actions**: não possui.

## painelRelatorioGerencial

- **Descrição**: contém o código-fonte de todas as versões do Relatório Gerencial.

- **Link**: [https://github.com/ciga-mpmg/painelRelatorioGerencial](https://github.com/ciga-mpmg/painelRelatorioGerencial)

- **Actions**: não possui.

## painelActions

@azeloc

## painelDocumentacao

- **Descrição**: contém os arquivos que compõem esta documentação.

- **Link**: [https://github.com/ciga-mpmg/painelDocumentacao](https://github.com/ciga-mpmg/painelDocumentacao)

- **Actions**: não possui.

## scraper_sigbm

@azeloc
